...
从成本的角度来考量，人力成本应该是软件项目中最大的成本，甚至大于其他所有成本之和。软件公司的老板们会尽可能的压缩成本，就会减少人员投入、增加劳动时间、以初级程序员承担高级程序员的工作、有能力的程序员承担超过负荷的多头工作等等。
...
《人月神话》书中也指出：“系统调试（相对于单元测试）所花费的时间会比预料的更长”。针对这个问题，我在书中没有找到针对性的解决方案，我个人的建议只能期待于早集成、早暴露、早解决。

“开发是一时的，运维是长久的”，伴随着软件系统的完成上线，软件将转入“运维”阶段。随着用户的使用，“用户越多，发现的错误也越多”，而且各种所谓的优化改进建议、个性化要求也会接踵而来。
开发人员修复缺陷总会以20%-50%的机率引入新的BUG，产品生命周期中每月BUG数量先是下降，然后上升。这样的现象在45年前就已经存在了，而且还会长期存在下去，人们都在期望BUG会随着时间开始收敛，但是实际上“所有修改都倾向于破坏系统的架构，增加了系统的混乱程度（熵增加）”。这就需要每次修复之后，必须重新运行之前所有的测试用例。

 一个应用软件运行维护几年之后，再进行需求修改或者BUG修复都会是一件极度头疼的工作，最后，整个软件系统不得以“升级改造”的名义推翻重来。然后，所有问题清零了，开始了新的循环周期。

 在当今时代，软件的可维护性也成为了越来越重要的一项软件质量指标。在《人月神话》中也使用独立的一个章节来说明源代码以外的另外一面，这些内容包括：程序向用户所呈现的面貌（界面、文档、注释）与提供给机器识别的内容（代码）同样重要。不管是独立程序还是系统软件，都应该编写注释，因为人会遗忘的。注释需要描述事情如何，还应描述为什么，注释信息应该以段落的方式向程序中插入必要的记述性文字，同时可以借助源代码的固定格式来附件更多的文档信息并提升可读性。

# 十一 讨厌的文档
在软件项目过程中，提到“文档”，人们的脑海中就会产生一些不好的印象：海量的、无用的、脱节的、形式化的、厌烦的等等，但是这些文档不管出于什么原因，又是必不可少的。为了减少文档在编写和阅读过程中的抵触心理，提高软件人员对文档价值的认可，我们从以下几个方面总结文档的意义：
l  编写文档的过程是集中思考的过程，能够将各种意见明确出来，将各种矛盾和问题暴露出来。问题暴露才有可能解决，否则，项目会经常处于无休止的混乱状态中。

l  文档可以作为团队的沟通渠道，不管项目团队如何沟通，总会有一部分人对于一部分策略完全不知情，文档就是帮助团队成员统一认知，向着同一个方向前进。在团队成员发生变更、任务需要交接、工作需要多方配合等情况下，文档是最有效的机制，不仅能够帮助多方迅速形成结论，同时也作为多方承诺的有效记录。

l  文档是过程跟踪的有效机制，通过维护文档，明显地跟进项目任务的状态，同时，文档也可以作为检查列表等用途。文档可以直观化地说明项目的进度和状态，为项目计划及各种资源的调整提供有效依据。

l  文档会作为软件交付物不可或缺的一部分，在软件项目进行过程中，各个阶段性的评审需要软件作为评审交付物，在软件最终交付后，也会需要大量验收文档。同时交付大量用户文档，也能够减少用户培训和运维的成本。

项目经理的工作绝大部分都是围绕着各种文档展开，以文档为手段跟进项目状态、促进团队交流。其他团队成员，尤其是开发人员在编写代码的同时，如何分工编写文档？如何保证文档的及时更新?这些问题也制约着文档在软件项目中产生合理的价值。在《人月神话》中提出的外科手术式团队中，由专门的编辑人员负责所有文档的编写和管理。即使不能组建这样豪华的团队，我也深刻赞同专人负责文档的机制。

# 十二 瀑布、敏捷
在《人月神话》20周年纪念版的内容中直接否定了“瀑布模型”，提倡采用增量开发模型：软件系统是逐渐增长得到的，而不是搭建完成的。在45年后的今天，出现了大量持续集成方案用来支撑系统的快速迭代。
可抛弃原型系统在45年前似乎还是比较奢侈的行为，但是，作者已经认识到这种机制所能带来的对软件系统的巨大优势。由于软件的不可见性，软件在没有完成之前太过于抽象，以至于软件行业存在一句话：只有当你做完了，你才知道你要做什么。而可抛弃原型实际上就是快速模拟完成一次系统开发的过程。拿出一个原型系统，用户才知道真正想要哪些功能，架构师才知道系统的完整概念是什么，开发人员才知道要开发什么功能，这些都是原型系统存在的价值，它的存在会避免很多需求不清、反复返工的过程。可抛弃又代表了这个原型系统遇到的问题、过程中存在妥协，可以有一次重新整体设计的过程，不必受到这个原型系统的约束。“Axure”为代表的交互原型设计软件在这方面体现出了存在的意义。

原型系统完成之后，开始正式系统的设计开发，系统不是拆分成独立模块，都开发完成后再进行组装的，而是迅速实现一个可运行的版本，然后不停的按照计划好的周期更新功能，发布版本，每个版本都是可以运行的完整系统，系统持续优化完善，功能不断增长，只要用户有需要，这个过程可以长期运转下去。

渐进式演变可以以最快的速度交付给用户一个可运行的系统，最早得到用户真实的使用反馈，可以在第一时间根据用户反馈进行修复和针对性完善，

应用系统一直以一个节奏进行持续改善，每次版本都包含完整系统，这样的话，模块之间的集成和系统之间的集成也不会拖到最后时刻才能发现问题。

版本发布的周期没有固定的规则，需要根据用户环境条件，技术团队的储备等各方资源进行合理评估和调整。

渐进性演进开发不是没有代价的，首先，伴随着版本发布的常态化，各个工种的团队人员需要时刻保持紧张配合，对人员和流程的严谨性提出了更高的要求；其次，每个版本都是完整的功能，每次发布都需要全面测试，确保之前版本已经正确的功能不能在新版本中出现错误，面对这种情况，蓝绿发布、灰度发布也是一种妥协的折中方案；最后，系统不断演进变化，初期系统不能预料后期开发的功能，这就对软件系统的设计水平提高了要求，软件的开闭原则提出：软件或者模块应该对扩展开放，对扩展关闭，要求在不修改原有功能的情况下扩展新功能。这是一个庞大的话题，设计开发人员学习各种软件设计原则，设计模式，开发框架都是为了这个目标。

# 十三 沟通沟通沟通
《人月神话》以巴比伦塔建设失败的的神话故事引出了“沟通”重要性。当上帝消除了统一的语言，人们失去了交流的手段，即使其他条件都能满足，巴比伦塔也以失败告终。”沟通”是确保团队思想统一、目标一致的最重要手段。
那么，如何进行有效的沟通：

一是时常保持非正式的交流沟通。比如面对面口头的交流或者电话等远程交流，不论哪种形式，交流的内容都应形成文字记录。
